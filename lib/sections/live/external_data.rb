    ...
    class ExternalData < Data
      ...
      def add_warcraftlogs_data
        RAID_DIFFICULTIES.each_key do |diff|
          ...
          WCL_IDS[:live].each do |boss|
            value = @character.details['warcraftlogs_awakened'][diff.to_s][boss]
      ...
      def add_leaderboard_data
        ...
        if GREAT_VAULT_BLACKLISTED_PERIODS.include?(Audit.period)
          ...
          @character.data['great_vault_slot_4'] = GREAT_VAULT_TO_ILVL['dungeon'][[dungeon_data[0] || 0, 10].min] || ""
          @character.data['great_vault_slot_5'] = GREAT_VAULT_TO_ILVL['dungeon'][[dungeon_data[3] || 0, 10].min] || ""
          @character.data['great_vault_slot_6'] = GREAT_VAULT_TO_ILVL['dungeon'][[dungeon_data[7] || 0, 10].min] || ""
        end
      end
    end
  end
