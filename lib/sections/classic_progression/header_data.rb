  ...
  module ClassicProgression
    class HeaderData
      ...
      def self.altered_header(team)
        new_header = HEADER[:classic_progression][0 .. HEADER[:classic_progression].length] #Copy the original header
        ...
        # Parse messages
        new_header[4] = "#{CURRENT_VERSION[:classic_progression]}|#{VERSION_MESSAGE}"
        ...
        data = []
        VALID_RAIDS[:classic_progression].each do |raid|
