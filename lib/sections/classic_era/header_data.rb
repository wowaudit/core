  ...
  module ClassicEra
    class HeaderData
      ...
      def self.altered_header(team)
        new_header = HEADER[:classic_era][0 .. HEADER[:classic_era].length] #Copy the original header
        ...
        # Parse messages
        new_header[4] = "#{CURRENT_VERSION[:classic_era]}|#{VERSION_MESSAGE}"
        ...
        data = []
        VALID_RAIDS[:classic_era].each do |raid|
