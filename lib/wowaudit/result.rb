module Wowaudit
  class Result
    attr_accessor :output, :data, :gems, :ilvl, :character, :changed, :details, :stat_info, :delve_info, :prey_info
    ...
    def gdpr_deletion?
      ...
      if @character.profile_id != profile_id
        if @character.realm.kind == 'classic_era' || @character.realm.kind == 'classic_anniversary' || @character.achievement_uid != new_achievement_uid
    ...
    def check_data_completeness
      ...
      if @character.realm.kind != 'classic_era' && @character.realm.kind != 'classic_anniversary' && (Wowaudit.extended || Wowaudit.extra_fields.include?(:achievements))
