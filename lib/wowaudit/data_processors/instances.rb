  ...
  class DataProcessor::Instances < DataProcessor::Base
    def add
      ...
      begin
        ...
        dungeons_and_raids['sub_categories'].map{ |cat| cat['statistics'] }.flatten.each do |instance|
          if MYTHIC_DUNGEONS.include?(instance['id'])
            @result.data[MYTHIC_DUNGEONS[instance['id']]] = instance['quantity'].to_i
      ...
      unless total_dungeons.zero?
        @result.data['dungeons_done_total'] = total_dungeons
      end

      @result.data['m+_score'] = @data[:season_keystones].dig('mythic_rating', 'rating') || ''
      ...
      MYTHIC_DUNGEONS.values.each do |dungeon|
        @result.data["#{dungeon}_score"] = best_runs[dungeon]
      end
      ...
      raid_output.each do |metric, data|
        @result.data[metric] = data.join('|')
      end

      add_great_vault_data(great_vault_list)

      if @achievements
        @result.data['cutting_edge'] =
          CUTTING_EDGE_ACHIEVEMENTS.count{ |raid| @achievements[raid] }
        @result.data['ahead_of_the_curve'] =
          AHEAD_OF_THE_CURVE_ACHIEVEMENTS.count{ |raid| @achievements[raid] }

        total_layers = 0
        @achievements[14810]['criteria']['child_criteria'].each do |wing|
          @result.data["torghast_layers_#{TORGHAST_WINGS[wing['id']]}"] = TORGHAST_LAYER_CRITERIA.count { |layer| layer <= (wing['amount'] || 0) }
          total_layers += @result.data["torghast_layers_#{TORGHAST_WINGS[wing['id']]}"]
        end rescue nil

        @result.data['torghast_layers_twisting_corridors'] =
          TORGHAST_TWISTING_CORRIDORS_IDS.count{ |layer| @achievements[layer] }

        @result.data['torghast_floors'] = total_layers + @result.data['torghast_layers_twisting_corridors']
      end
    end
    ...
    def add_great_vault_data(raid_completions)
      ...
      GREAT_VAULT_RAID_KILLS_NEEDED.each do |slot, kills_needed|
        @result.data["great_vault_slot_#{slot}"] = if raid_bosses_killed >= kills_needed
      ...
      @result.data['great_vault_slot_4'] = 0 #GREAT_VAULT_TO_ILVL['dungeon'][[dungeon_data[0] || 0, 15].min]
      @result.data['great_vault_slot_5'] = 0 #GREAT_VAULT_TO_ILVL['dungeon'][[dungeon_data[3] || 0, 15].min]
      @result.data['great_vault_slot_6'] = 0 #GREAT_VAULT_TO_ILVL['dungeon'][[dungeon_data[9] || 0, 15].min]
      ...
      @result.data['great_vault_slot_7'] = honor_earned >= 1250 ? item_level : ''
      @result.data['great_vault_slot_8'] = honor_earned >= 2500 ? item_level : ''
      @result.data['great_vault_slot_9'] = honor_earned >= 6250 ? item_level : ''
    end
  end
end
