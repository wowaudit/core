...
$eu = ["Aegwynn",  "Aerie Peak",  "Agamaggan",  "Aggra (Português)",  "Aggramar",  "Ahn'Qiraj",  "Al'Akir",  "Alexstrasza",  "Alleria",
         ...
         "Vek'nilash",  "Vol'jin",  "Wildhammer",  "Wrathbringer",  "Xavius",  "Ysera",  "Ysondre",  "Zenedar", "Ревущий фьорд", "Гордунни", "Свежеватель Душ", "Ясеневый лес", "Черный Шрам", "Разувий", "Страж смерти", "Дракономор", "Азурегос", "Борейская тундра", "Вечная Песня", "Пиратская Бухта", "Голдринн", "Король-лич", "Галакронд", "Гром","Свежеватель-Душ",
...
$go = true;
if ($_POST['guild_region'] == 'EU') {
$valid_realms = array_map('strtolower',$eu); } elseif ($_POST['guild_region'] == 'US') {
$valid_realms = array_map('strtolower',$us);};
...
$json_data = json_decode(curl_exec($curlSession),true);
curl_close($curlSession);

if ($json_data['status'] == 'nok') {
header("Location: new.php?fail=noexist");
die();
}
...
$result = mysqli_query($con,"SELECT * FROM guilds WHERE LOWER(name) = LOWER('" . $guild_name . "') AND realm = '" . ucfirst($_POST['guild_realm']) . "'");


if (mysqli_num_rows($result)==0) {

$keycode = mb_substr(str_shuffle("abcdefghjkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ123456789"), 0, 1).substr(md5(time()),1,9);

$new_guild = mysqli_query($con,"INSERT INTO guilds (name,region,realm,key_code) VALUES ('" . $guild_name . "', '" . $_POST['guild_region'] . "', '" . str_replace("'","\'",ucfirst($_POST['guild_realm'])) . "', '" . $keycode ."')");

} else {
header("Location: new.php?fail=taken");
die();
}
...
if ($go) {
header("Location: manage.php?key=" . $keycode);
die();

} else {
header("Location: new.php?fail=" . $_POST['guild_region']);
die();
}
