...
if team_ids:
    ...
    execute_query('UPDATE characters SET raiderio_weekly = NULL WHERE team_id IN ({0})'.format(','.join(team_ids)))
    ...
    if region == 'US' and datetime.datetime.weekday(datetime.datetime.now()) != 1:
        ...
        execute_query('UPDATE characters SET raiderio_weekly = NULL WHERE team_id IN (SELECT id from teams WHERE guild_id IN (SELECT id FROM guilds WHERE region = \'{0}\'))'.format(region))
