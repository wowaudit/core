...
class Scraper(object):
    ...
    def allocate(self):
        allocations = MAX_ALLOCATED if self.mode not in ['snapshot_EU','snapshot_US'] else 1000000
        guild_data = [str(guild[0]) for guild in execute_query('SELECT guild_id FROM guilds WHERE patreon = {0} ORDER BY last_checked ASC LIMIT {1}'.format(1 if self.mode == 'production_patreon' else 0, allocations))]
        ...
        for member in data:
            ...
            if key_code not in self.done:
                if self.mode == 'snapshot_EU':
                    self.guilds.append(Guild(member,self.mode,'EU'))
                elif self.mode == 'snapshot_US':
                    self.guilds.append(Guild(member,self.mode,'US'))
                else:
                    self.guilds.append(Guild(member,self.mode))
